...
import { Users, Search, ShieldCheck, ShieldAlert, Check, X, MapPin } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900">Manage Registered Farmers</h1>
          <p className="text-xs text-slate-500 mt-1">
            Oversee grower accounts, landholdings, verification status, and moderation.
          </p>
        </div>
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            placeholder="Search farmer name, district, email..."
              ...
              {filteredFarmers.map((user) => (
                <tr key={user._id} className="hover:bg-slate-50/60 transition">
                  ...
                  <td className="py-3.5 px-4 text-slate-600 font-medium">
                    {user.profile?.village ? `${user.profile.village}, ` : ""}
                    {user.profile?.district || "-"}, {user.profile?.state || "-"}
                  </td>
                  <td className="py-3.5 px-4 font-semibold text-slate-800">
                    {user.profile?.farmSize || "-"} {user.profile?.farmSizeUnit || "Acres"}
                  </td>
