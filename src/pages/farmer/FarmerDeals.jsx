  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-3xl bg-white border border-slate-200/80 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900">Deals & Price Agreements</h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Confirmed trade agreements and incoming/outgoing price negotiation offers.
          </p>
        </div>

        <div className="p-3 bg-emerald-50 rounded-2xl border border-emerald-200 text-right shrink-0">
          <span className="text-[10px] uppercase font-bold text-emerald-800 tracking-wider block">
            Total Trade Value Confirmed
          </span>
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-2">
        ...
        >
          ...
          <span>Active Offers & Negotiations ({offers.length})</span>
        </button>
      </div>
        ...
        <>
          {deals.length === 0 ? (
            <EmptyState
              ...
              description="When a company accepts your crop offer or you accept a company's proposal, confirmed deals will appear here."
            />
          ) : (
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5 text-xs text-slate-600 my-3">
                      <div className="flex items-center justify-between">
                        <span className="text-slate-400">Buyer Company:</span>
        ...
        <>
          {offers.length === 0 ? (
            <EmptyState
              ...
              description="Send proposals on company crop demands or respond to direct buyer offers in messages."
            />
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {offers.map((offer) => (
        ...
        >
          <form onSubmit={handleUpdateStatusSubmit} className="space-y-4">
            <div>
              ...
              >
                <option value="confirmed">Confirmed (Agreement Locked)</option>
                <option value="in_progress">In Progress (Harvesting / Packing / En Route)</option>
                <option value="completed">Completed (Goods Received & Payment Settled)</option>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Logistics / Status Notes</label>
              <textarea
                rows="3"
                value={dealNotes}
                onChange={(e) => setDealNotes(e.target.value)}
                placeholder="e.g. Dispatched 50 crates via local truck #TN-67-1234..."
                className="w-full px-3.5 py-2 rounded-xl border border-slate-200 text-sm"
              />
            </div>
